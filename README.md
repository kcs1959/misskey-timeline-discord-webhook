...
| `WITH_FILES`               | いいえ   | ファイル付きノートのみ購読する（既定: `false`）                  |
| `INCLUDE_ATTACHMENTS`      | いいえ   | 添付ファイル情報を Discord に含める（既定: `true`）              |
...
`WITH_FILES` は Misskey ストリーミング API の `withFiles` パラメータをそのまま渡します（`true` のときファイル付きノートのみ受信）。Discord に送るメッセージに添付ファイル（embed・リンク）を含めるかどうかは `INCLUDE_ATTACHMENTS` で制御します。
